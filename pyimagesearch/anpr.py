...
class PyImageSearchANPR:
    ...
    def locate_license_plate_candidates(self, gray, keep=5):
        ...
        # compute the Scharr gradient represenation of the blackhat image in the x-direction and then scale the result back to range [0, 255]
        ...
        # blur the gradient representaiton, applying a closing operation, and threshold the image using Otsu's method
        gradX = cv2.GaussianBlur(gradX, (5,5),0)
        gradX = cv2.morphologyEx(gradX, cv2.MORPH_CLOSE, rectKern)
        thresh = cv2.threshold(gradX, 0,244, cv2.THRESH_BINARY | cv2.THRESH_OTSU)[1]
        self.debug_imshow("Grad Thresh", thresh)
        
        # perform a series of erosion and dilations to clean up the threshold image
        ...
        thresh = cv2.dilate(thresh, None, None, iterations=2)
        thresh = cv2.erode(thresh, None, iterations=1)
        self.debug_imshow("Final", thresh, waitKey=True)
        
        # fight contours in the thresholded image and sort them by their size in descending order, keeping only the largest ones
    ...
    def locate_license_plate(self, gray, candidates, clearBorder=False):
        ...
        for c in candidates:
            ...
            if ar >= self.minAR and ar <= self.maxAR:
                ...
                # check to see if we should clear any foreground pixels touching the border of the image which typically but not always indicate  noise
                if clearBorder:
    ...
    def find_and_ocr(self, image, psm=7, clearBorder=False):
        ...
        lpText = None
        # convert the input image to grayscale, locate all candidate license plate regions in the image, and then process the candidates, leaving us with the actual license plate
        ...
        # return a 2 tuple of the OCR'd license plate text along with the contour associated with the license plate region
        return (lpText, lpCnt)
